...
// MakeMusicChannelsString moved to xbmc/guilib/guiinfo/MusicChannelsHelper.h
// so VideoGUIInfo (VideoPlayer.MusicChannelsString) can share the same logic.
                             ...
                             std::string* fallback) const
{
  ...
  if (tag)
  {
    switch (info.GetInfo())
    {
      ...
      case MUSICPLAYER_MUSIC_CHANNELS_STRING:
      {
        ...
        // a tag-side URL (rare; defensive). Same handler for both list
        // browsers and PaPlayer now-playing — item + tag come pre-bound
        // to the right object by the InfoLabel resolver.
