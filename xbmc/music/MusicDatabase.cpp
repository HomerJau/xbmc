    ...
    if (m_pDS->num_rows() == 0)
    {
      ...
      strSQL =
          PrepareSQL("INSERT INTO album (idAlbum, strAlbum, strArtistDisp, strGenres, "
                     ...
                     bCompilation, releaseType.ToStdString().c_str());
                                ...
                                bool bScrapedMBID)
{
  ...
  strSQL = PrepareSQL("UPDATE album SET "
                      ...
                      releaseType.ToStdString().c_str(), strReleaseStatus.c_str(), //
...
void CMusicDatabase::UpdateTables(int version)
{
  ...
  if (version < 86) // add streamdetails table for the Audio-Streams feature
  {
    // Only the table itself goes here. The indexes for streamdetails, the
    // streamdetails-cleanup additions to tgrDeleteAlbum / tgrDeleteSong, and
    // the three new sibling views (albumstreamview / albumvirtualview /
    // songvirtualview) are all in CreateAnalytics() / CreateViews(), which
...
int CMusicDatabase::GetConcertsCount()
{
  std::string strSQL =
      PrepareSQL("SELECT COUNT (DISTINCT idAlbum) FROM album WHERE strReleaseType like '%s'",
                 AudioType::ToStdString(AudioType::Content::Concert).c_str());
  return GetSingleValueInt(strSQL);
}
...
void CMusicDatabase::SetPropertiesFromAlbum(CFileItem& item, const CAlbum& album)
{
  ...
  item.SetProperty("album_releasetype", AudioType::ToStdString(album.releaseType));
